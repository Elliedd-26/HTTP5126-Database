...
WHERE role = 'Manager' OR role = 'Assistant'
-- B
SELECT * FROM stock_item
WHERE category = 'Murine' AND inventory < 24; 
...
--  4
-- A
SELECT last_name, first_name, phone FROM employee
WHERE role NOT IN ('assistant', 'Manager');
-- B
SELECT item, price, inventory FROM stock_item
WHERE category NOT IN ('Canine', 'Feline') 
AND inventory <= 20;
...
WHERE category IN ('Murine') AND inventory > 20
ORDER BY price DESC;