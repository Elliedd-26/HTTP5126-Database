...
-- B
SELECT last_name AS "Pet Store Staff",
employee_id AS "Emp. ID", SIN FROM employee
...
-- B
SELECT item AS "Name", stock_item_id AS "ID", inventory 
FROM stock_item 
WHERE inventory <= 12;

--  5
-- A
SELECT item AS "Kitty Cat", price 
FROM stock_item 
WHERE category = "Feline";
-- B
SELECT CONCAT(first_name, ' ', last_name) AS "Employee Member" 
FROM employee
